...
/* ==================================================
  Metaboxes options
================================================== */
function meloo_toolkit_mb_videos() {
	...
	/* Meta info */ 
	$meta_info = array(
		'title' => esc_html__( 'Video Options', 'meloo-toolkit'), 
		'id'    =>'rascals_mb_videos_box', 
		'page'  => array(
			'meloo_videos'
		), 
		'context'  => 'normal', 
	...
	/* Box Filter */
	if ( has_filter( 'rascals_mb_videos_box' ) ) {
		$meta_info = apply_filters( 'rascals_mb_videos_box', $meta_info );
	}
	...
	$meta_options = array(
		...
		array(
			...
			/* Video URL */
			array(
				'name'      => esc_html__( 'Video URL', 'meloo-toolkit' ),
				'id'        => '_video_url',
				'type'      => 'text',
				'std'       => '',
				'separator' => true,
				'desc'      => esc_html__( 'Add Youtube or Vimeo link here. e.g: https://www.youtube.com/watch?v=lMJXxhRFO1k', 'meloo-toolkit' )
			), 
	...
	/* Options Filter */
	if ( has_filter( 'rascals_mb_videos_opts' ) ) {
		$meta_options = apply_filters( 'rascals_mb_videos_opts', $meta_options );
	}

	/* Add class instance */
	$rascals_mb_videos = new RascalsBox( $meta_options, $meta_info );
	...
	/* Box Filter */
	if ( has_filter( 'rascals_mb_videos_template_box' ) ) {
		$meta_info = apply_filters( 'rascals_mb_videos_template_box', $meta_info );
	}
	...
	$meta_options = array(
		...
		array(
			...
			/* Video layout */
			array(
				'name' => esc_html__( 'Video Layout', 'meloo-toolkit' ),
				'id'   => '_videos_layout',
				'type' => 'select_image',
				'std'  => 'wide',
				'images' => array(
				...
				'desc' => esc_html__( 'Choose the videos layout. The settings from this panel applies to the bottom of the page, where the loop and sidebar is.', 'meloo-toolkit' )
			),
		...
		array(
			...
			/* Click Action */
			array(
				'name' => esc_html__( 'Click Action', 'meloo-toolkit' ),
				'id'   => '_click_action',
				'type' => 'select',
				'std'  => 'open_in_player',
				'options' => array(
					array( 'name' => esc_html__( 'Open video in player', 'meloo-toolkit' ), 'value' => 'open_in_player' ),
					array( 'name' => esc_html__( 'Open video in popup window', 'meloo-toolkit' ), 'value' => 'open_in_lightbox' ),
					array( 'name' => esc_html__( 'Open video on the new page', 'meloo-toolkit' ), 'value' => 'open_on_page' )
				),
				'desc' => esc_html__( 'Open video in popup window or on the page.', 'meloo-toolkit' )
			),
	...
	/* Options Filter */
	if ( has_filter( 'rascals_mb_videos_template_opts' ) ) {
		$meta_options = apply_filters( 'rascals_mb_videos_template_opts', $meta_options );
	}

	/* Add class instance */
	$rascals_mb_videos_template = new RascalsBox( $meta_options, $meta_info );
		
}

return meloo_toolkit_mb_videos();