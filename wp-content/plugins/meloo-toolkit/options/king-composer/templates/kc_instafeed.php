...
<div class="<?php echo esc_attr( implode(' ', $wrap_class) ); ?> <?php echo esc_attr( $atts['classes'] ); ?>">
    ...
    if ( false === ( $cache_task = get_transient( $cache_key ) ) ) {
        ...
        if ( is_wp_error( $remote_images ) || is_wp_error( $remote_user ) ) {
             // error handling 
            return esc_html_e( "Something went wrong with Instagram server", 'meloo-toolkit' ) . $error_message;
        }
    ...
    <?php if ( ! empty( $atts['display_header'] ) ) : ?>
        <div class="rt-instagram-header">
            ...
            <div class="rt-instagram-data">
                ...
                <a class="rt-instagram-button" target="_blank" href="https://www.instagram.com/<?php echo esc_attr( $atts['username'] ); ?>"><?php esc_html_e( 'Follow', 'meloo-toolkit' ); ?></a>
            </div>
        </div>
    <?php endif; ?>
            ...
            <?php if ( isset( $image->link )) : ?>
                <div class="rt-instagram-image">
                    ...
                    <a href="<?php echo esc_url( $image->link ) ?>" target="_blank">
                        <img src="<?php echo esc_url( $image->images->standard_resolution->url ) ?>" alt="<?php echo esc_attr__( 'Instagram image', 'meloo-toolkit' ); ?>">
                    </a>
                </div>
                    <div class="rt-instagram-meta">
                        <div>
                            <span class="rt-instagram-comments"><i class="icon icon-comment-o"></i><?php echo meloo_format_number($image->comments->count ); ?></span>
                            <span class="rt-instagram-likes"><i class="icon icon-heart-o"></i><?php echo meloo_format_number( $image->likes->count ); ?></span>
                        </div>
                    </div>
