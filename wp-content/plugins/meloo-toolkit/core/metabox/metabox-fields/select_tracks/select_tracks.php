        ...
        */
		public function render() {
			...
			if ( isset( self::$_option['id'][0]['id'] ) ) {
				...
				return esc_html( rascals_esc_( 'Error: ID: Is not defined!' ) );
			}
					...
					echo '<div class="box-tc box-tc-input">';
						...
						echo '<div class="box-tp-block">';
							...
							if ( $track_std !== 'none' ) {
								...
								$tracklist = $this->get_list( $track_std, $tracks_std );
						...
						if ( $track_std !== 'none' ) {
							echo '<a href="' . admin_url( 'post.php?post=' . esc_attr( $track_std ) . '&action=edit' ) . '" class="edit-track-post" target="_blank">' . esc_html( rascals_esc_( 'Edit original tracklist here.' ) ) . '</a>';
						}
		...
		public function get_tracks_ajax() {
			...
			$tracklist = $this->get_list( $id );

			$output = $this->tracks_render( $tracklist );

			$output .= '<a href="' . admin_url( 'post.php?post=' . esc_attr( $id ) . '&action=edit' ) . '" class="edit-track-post" target="_blank">' . esc_html( rascals_esc_( 'Edit original tracklist here.' ) ) . '</a>';
		...
		/* Get List
		---------------------------------------------- */
		public function get_list( $audio_post, $custom_ids = '') {
			...
			if ( is_array( $tracks ) && ! empty( $tracks ) ) {
				foreach ( $tracks as $track ) {
			       	if ( ! $track['cover'] || $track['cover'] === '' ) {
			            $track['cover'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
			        }
			        $output .= '
			        <div class="track-item" data-id="' . esc_attr( $track['id'] ) . '">
			            ...
			            <a href="#" class="remove-track" title="' . esc_html( rascals_esc_( 'Remove Track' ) ) . '"></a>
			        </div>';
			    }
