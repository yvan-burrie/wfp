...
if ( ! class_exists( 'RascalsBox_select_array' ) ) {

	class RascalsBox_select_array extends RascalsBox {
						...
						echo '<select name="' . esc_attr( self::$_option['id'] ) . '" id="' . esc_attr( self::$_option['id'] ) . '" size="1"  class="box-select">';
							if (isset( self::$_option['options'] ) ) {
								foreach ( self::$_option['options'] as $option ) {
									if ( isset( self::$_option['std'] ) && self::$_option['std'] === strval( $option['value']) ) $selected = 'selected';
									else $selected = '';
							...
							if ( isset( self::$_option['array'] ) && isset( self::$_option['key'] ) ) {
								...
								if ( is_array( $custom_array ) ) {
									foreach ( $custom_array as $array ) {
										if ( self::$_option['std'] === strval( $array[$key] ) ) { 
											$selected = 'selected';
										} else {
											$selected = '';
										}
										...
										echo "<option " . esc_attr( $selected ) ." value=\"" . esc_attr( $array[$key] ) . "\">" . esc_attr( $name ) . "</option>" . "\n";
									}
								}
							}
						echo '</select>';
