	...
	class RascalsBox_bg_generator extends RascalsBox {
        ...
        */
		public function enqueue() {
	    	...
	    	$path = self::$_args['admin_url'] . '/core/metabox';
        ...
        */
			...
			if ( isset( self::$_option['dependency']) && is_array( self::$_option['dependency'] ) ) {
				...
				echo '<div class="box-row-input">';
					...
					echo '<div class="box-tc box-tc-input">';
						...
						echo '<div class="bg-holder-wrap" data-add-label="' . esc_attr( rascals_esc_( 'Add Background' ) ) . '" data-edit-label="' . esc_attr( rascals_esc_( 'Edit Background' ) ) . '">';
						...
						if ( !isset( self::$_option['std'] ) || self::$_option['std'] === '' ) {
							...
							echo '<button class="_button generate-bg" ><i class="fa icon fa-magic"></i>' . esc_html( rascals_esc_( 'Add Background' ) ) . '</button>';
							echo '<button class="_button ui-button-delete delete-bg" style="display:none"><i class="fa icon fa-trash-o"></i>' . esc_html( rascals_esc_( 'Remove' ) ) . '</button>';
						} else {
							...
							if ( json_decode( $data ) !== null ) {
								...
								if ( isset( $data['image'] ) ) {
									...
									// If image exists
									if ( $image ) {
										$preview .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr( rascals_esc_( 'Preview Image' ) ) . '">';
									}
								} 
								if ( isset( $data['color'] ) ) {
								...
								$messages = '<div class="msg msg-warning">' . esc_html( rascals_esc_( 'Set your background again, because current is not compatible with latest framework version.' ) ) . '</div>';
							}
							echo '<div class="image-holder bg-image-holder">';
							$this->e_esc( $preview );
							echo '</div>';
							$this->e_esc( $messages );
							echo '<button class="_button generate-bg" ><i class="fa icon fa-magic"></i>' . esc_html( rascals_esc_( 'Edit Background' ) ) . '</button>';
							echo '<button class="_button ui-button-delete delete-bg"><i class="fa icon fa-trash-o"></i>' . esc_html( rascals_esc_( 'Remove' ) ) . '</button>';
						}
		...
		public function bg_editor() {
			...
			<div class="box-row clearfix">
				<div class="box-row-input">
					<div class="box-tc box-tc-label">
						<label for="color">' . esc_html( rascals_esc_( 'Background Color' ) ) . '</label>
					</div>
			...
			<div class="box-row clearfix">
				<div class="box-row-input">
					<div class="box-tc box-tc-label">
						<label for="bg-image">' . esc_html( rascals_esc_( 'Background Image' ) ) . '</label>
					</div>
					...
					<div class="box-tc box-tc-input">
						...
						// If image exists
						if ( $image ) {
							$image_html = '<img src="' . esc_attr( $image ) . '" alt="' . esc_attr( rascals_esc_( 'Preview Image' ) ) . '">';
							$is_image = 'is_image'; 
						} else {
							$image_html = '';
							$is_image = ''; 
			...
			$output .= '<div class="box-row clearfix">';
				$output .= '<div class="box-row-input">
						<div class="box-tc box-tc-label">
							<label for="position">' . esc_html( rascals_esc_( 'Background Position' ) ) . '</label>
						</div>
					...
					<div class="box-tc box-tc-input">';
						...
						$output .= '<p class="help-box">' . esc_html( rascals_esc_( 'The background-position property sets the starting position of a background image. The first value is the horizontal position and the second value is the vertical. The top left corner is 0 0.' ) ) . '</p>';
			...
			$output .= '<div class="box-row clearfix">';
				$output .= '<div class="box-row-input">
						<div class="box-tc box-tc-label">
							<label for="repeat">' . esc_html( rascals_esc_( 'Background Repeat' ) ) . '</label>
						</div>
					...
					<div class="box-tc box-tc-input">';
						...
						$output .= '<p class="help-box">' . esc_html( rascals_esc_( 'The background-repeat property sets if/how a background image will be repeated.' ) ) . '</p>';
						unset( $select_options );
					$output .= '</div>';
				$output .= '</div>';
				$output .= '<div class="box-row-line"></div>';
			...
			$output .= '<div class="box-row clearfix">';
				$output .= '<div class="box-row-input">
						<div class="box-tc box-tc-label">
							<label for="attachment">' . esc_html( rascals_esc_( 'Background Attachment' ) ) . '</label>
						</div>
					...
					<div class="box-tc box-tc-input">';
						...
						$output .= '<p class="help-box">' . esc_html( rascals_esc_( 'The background-attachment property sets whether a background image is fixed or scrolls with the rest of the page.' ) ) . '</p>';
						unset( $select_options );
					$output .= '</div>';
				$output .= '</div>';
				$output .= '<div class="box-row-line"></div>';
			...
			$output .= '<div class="box-row clearfix">';
				$output .= '<div class="box-row-input">
						<div class="box-tc box-tc-label">
							<label for="size">' . esc_html( rascals_esc_( 'Background Size' ) ) . '</label>
						</div>
					...
					<div class="box-tc box-tc-input">';
						...
						$output .= '<p class="help-box">' . esc_html( rascals_esc_( 'The background-size property specifies the size of the background images.' ) ) . '</p>';
						unset( $select_options );
					$output .= '</div>';
				$output .= '</div>';
				$output .= '<div class="box-row-line"></div>';
