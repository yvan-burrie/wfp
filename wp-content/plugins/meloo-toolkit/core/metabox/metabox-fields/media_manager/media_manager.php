	...
	class RascalsBox_media_manager extends RascalsBox {
        ...
        */
		public function render() {
			...
			$def_img_src = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
			...
			if ( isset( self::$_option['dependency']) && is_array( self::$_option['dependency'] ) ) {
				...
				if ( isset( self::$_option['name'] ) && ( self::$_option['name'] !== '' ) ) {
					...
					echo '<div class="box-tc box-tc-input">';
						...
						/* Select All Items */
						echo '<a title="' . esc_html( rascals_esc_( 'Select All' ) ) . '" class="mm-select-all"><i class="fa fa-check-square"></i></a>';
						...
						if ( self::$_option['layout'] === 'list' ) {
							...
							/* Head list TPL
							 -------------------------------- */
							$default_item = array();
							$default_item['cover'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
								...
								if ( self::$_option['layout'] === 'grid' ) {
									...
									if ( $image ) {
										...
										<a class="mm-item mm-image" id="' . esc_attr( $id ) . '">
											<div class="mm-item-preview">
										    	<div class="mm-filename"><div>' . esc_html( rascals_esc_( 'Error: Image file doesn\'t exists.' ) ) . '</div></div>
											</div>
										</a>';
									}
								}
									...
									if ( $list_item ) {
										...
										if ( isset( $item[ 'image' ] ) ) {
											...
											if ( $src === false || $src === null || $src === '' ) {
												$src = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
											}
											$item['image'] = $src;
										}
						...
						echo '<div class="mm-list-tpl">';
							$default_item = array();
							$default_item['cover'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
							$default_item['image'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
						...
						/* Messages
						 -------------------------------- */
						echo '<p class="msg msg-error" style="display:none;">' . esc_html( rascals_esc_( 'Error: AJAX Transport' )) . '</p>';
						...
						/* Delete */
						echo '<button class="_button _button-red mm-delete-button" title="' . esc_html( rascals_esc_( 'Remove Selected Items' ) ) . '" style="display:none">' . esc_html( rascals_esc_( 'Remove Selected' ) ) . '</button>';
			...
			if ( isset( $display_fields ) && is_array( $display_fields ) && is_array( $data_fields ) ) {
			 	...
			 	foreach ( $display_fields as $i => $f ) {
			 		...
			 		if ( $f['type'] === 'image' ) {
			 			...
			 			if ( ! $title ) {
				 			if (  ! isset($new_field )) {
				 				$new_field = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
				 			}
		...
		function list_tpl( $id, $fields ) {
			...
			return	'<div class="mm-item" id="' . esc_attr( $id ) . '">
						...
						<div class="mm-field mm-field-buttons">
				            <a title="' . esc_html( rascals_esc_( 'Save' ) ) . '" class="mm-save-button"><i class="fa fa-save"></i></a>
				            <a title="' . esc_html( rascals_esc_( 'Edit/Close' ) ) . '" class="mm-edit-button"><i class="fa fa-gear"></i></a>
				            <a title="' . esc_html( rascals_esc_( 'Select' ) ) . '" class="mm-select-button"><i class="fa fa-check-square"></i></a>
			            </div>
		            </div>
				</div>';
		}
		...
		function mm_editor_save() {
			...
			if ( isset( $update_fields ) && $settings['mm_layout'] === 'list' ) {
				foreach ( $update_fields as $i => $field ) {
					...
					if ( $fields[$f] === '' && $field['type'] === 'image' ) {
						$update_fields[$i]['val'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
					} elseif ( $fields[$f] !== '' ) {
						if ( $field['type'] === 'image' ) {
							...
							if ( is_numeric( $fields[$f] ) ) {
								...
								if ( $image_cover ) {
									...
									$update_fields[$i]['val'] = self::$_args['admin_url'] . '/core/metabox/assets/images/audio.png';
								}
							} else {
				...
				if ( ! empty( $results ) ) {
					...
					foreach ( $results as $i => $result ) {
						...
						/* List */
							...
							/* Display list item
							 -------------------------------- */
							$result['cover'] = esc_url( self::$_args['admin_url'] ) . '/core/metabox/assets/images/audio.png';
		...
		function mm_explorer_box() {
			...
			echo '<input type="text" id="mm-search" name="mm-search" tabindex="60" autocomplete="off" value="" placeholder="' . esc_html( rascals_esc_( 'Search' ) ) . '" />';
			echo '</label>';
			echo '<label for="mm-select" class="mm-label-select">';
			echo '<span>' . esc_html( rascals_esc_( 'Select All:' ) ) . '</span>';
			...
			echo '</div>';
			echo '<div class="clear"></div>';
			echo '<span class="mm-load-next">' . esc_html( rascals_esc_( 'Load Next 30 Items' ) ) . '</span>';
		...
		public function mm_editor() {
			...
			if ( ! $item && ! $custom ) {
					echo '<p class="msg msg-error">' . esc_html( rascals_esc_( 'Error!' ) ) . '</p>';
				exit;
				return die();
			}

			if ( file_exists( self::$_args['admin_path'] . '/options/metaboxes/boxes/media-manager-panels.php' ) ) {
				$extra_fields = require_once( self::$_args['admin_path'] . '/options/metaboxes/boxes/media-manager-panels.php' );
				$panel .= $extra_fields;
			}
