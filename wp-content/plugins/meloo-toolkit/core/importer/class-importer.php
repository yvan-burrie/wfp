...
class RascalsImporter {
    ...
    public function __construct() {
        ...
        $this->demo_files_url = $this->demo_files_url;
    ...
    public function add_admin() {
        add_theme_page( rascals_esc_('Install Demos'), rascals_esc_('Install Demos'), 'edit_theme_options', 'admin-demos', array($this, 'demo_installer'));
    ...
    public function importer_enqueue() {
		...
		if ( strpos( $current_screen->base, 'admin-demos' ) !== false ) {
			...
			/* Style */
			wp_enqueue_style( 'rascals-importer', esc_url( RASCALS_TOOLKIT_URL ) . '/core/importer/assets/css/admin-importer.css' );

			/* Scripts */
			wp_enqueue_script( 'rascals-importer', esc_url( RASCALS_TOOLKIT_URL ) . '/core/importer/assets/js/admin-importer.js', false, false, true );
		}
    }
	...
	public function req_plugins_html() {
		...
		<div class="rascals-importer-warning">
		    <p class="tie_message_hint"><?php rascals_esc_e( 'Before you begin, you need to install and activate the following plugins') ?> (<a href="admin.php?page=admin-plugins"><?php rascals_esc_e( 'Install Plugins' );?></a>):</p>
		<ul>
  		<?php
    ...
    public function demo_installer() {
    	...
    	<h1><?php echo esc_html( $theme->get( 'Name' ) ); ?> <?php rascals_esc_e( 'Demos' ) ?></h1>
		<div class="about-text">
			<p>
			<?php rascals_esc_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch. Before running the importer, make sure that you have installed and activated the required and recommended plugins. You will see a message at the bottom of this page if you have not yet done so. When you import the data following things will happen:' ) ?>
			</p>
		</div>
		<ul class="rascals-list">
			<li><strong><?php rascals_esc_e( 'All widgets located in primary sidebar will be removed, please move them to another sidebar.' ) ?></strong></li>
	        <li><?php rascals_esc_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.' ) ?></li>
	        <li><?php rascals_esc_e( 'No WordPress settings will be modified.' ) ?></li>
	        <li><?php rascals_esc_e( 'Posts, pages, some images, some widgets and menus will get imported.' ) ?></li>
	        <li><?php rascals_esc_e( 'Images will be downloaded from our server.' ) ?></li>
	        <li><?php rascals_esc_e( 'Please click import only once and wait.' ) ?></li>
	        <li><strong><?php rascals_esc_e( 'Please be patient and wait for the import process to complete. It can take up to 5-7 minutes.' ) ?></li>
	      </ul>

		<div class="rascals-gdpr-confirm hidden">
			<input type="checkbox" id="confirm" name="confirm" value="confirm" />
    		<label for="confirm"><?php rascals_esc_e( 'I agree that all media elements of demo content like images, videos and mp3 files will be downloaded from the third-party server (rascalsthemes.com).' ) ?></label>
		</div>
       	...
       	<div class="rascals-importer-wrap" data-nonce="<?php echo wp_create_nonce('rascals-demo-code'); ?>">
	        ...
	        <form method="post">
								...
								<div class="demo-column">
									<?php
										...
										if ( $this->imported_demos && is_array($this->imported_demos) && isset( $this->imported_demos[$opt] ) ) {
											$btn_text = rascals_esc_( 'Import Again' );
											$demo_class = 'imported';
										} else {
											$btn_text = rascals_esc_( 'Import' );
											$demo_class = '';
										}
									 ?>
									<div class="demo <?php echo esc_attr( $demo_class ) ?>">
										<div class="demo-thumb">
											<img src="<?php echo esc_url( $this->demo_files_url ) . esc_attr( $option['thumb'] ) ?>" alt="Demo Thumb">
										</div>
				...
				<h3 class="rascals-importer-loading-msg hidden"><?php rascals_esc_e( 'Please be patient and wait for the import process to complete. It can take up to 5-7 minutes.' ); ?></h3>
				...
				<div id="rascals-importer-success">
					<h6><?php rascals_esc_e( 'Well Done! Demo content has been imported.' ) ?></h6>
    ...
    */
    ...
    public function set_demo_data( $file ) {
        ...
        if($importer_error){
            ...
            if ( ! is_file( $file )){
                ...
                rascals_esc_e( "The XML file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755. If this doesn't work please use the Wordpress importer and import the XML file (should be located in your download .zip: Sample Content folder) manually.");
                echo '</div>';
    ...
    public function set_demo_theme_options( $file ) {
		...
		if ( file_exists( $file ) ) {
			...
			if ( !empty( $data ) || is_array( $data ) ) {
      		...
      		wp_die(
  				rascals_esc_( 'Theme options Import file could not be found. Please try again.' ),
  				'',
    ...
    function process_widget_import_file( $file ) { 
    	...
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			rascals_esc_( 'Widget Import file could not be found. Please try again.' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = rascals_esc_( 'Sidebar does not exist in theme (using Inactive)');
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = rascals_esc_( 'Site does not support widget' ); // explain why widget not imported
    			}
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget === $check_widget ) {
    						...
    						$widget_message = rascals_esc_( 'Widget already exists' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = rascals_esc_( 'Imported' );
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = rascals_esc_( 'Imported to Inactive' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : rascals_esc_( 'No Title' ); // show "No Title" if widget instance is untitled
