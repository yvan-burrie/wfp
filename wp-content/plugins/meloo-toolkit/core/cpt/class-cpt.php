...
class RascalsCPT {
	...
	public function addAdminMenu() {	
		$panel_sub_page = add_submenu_page( 'edit.php?post_type=' . esc_attr( $this->args['post_name'] ), esc_attr( $this->args['post_name'] ) . '_sort', rascals_esc_( 'Sort Items' ), 'moderate_comments', $this->args['post_name'], array( $this, 'displaySortablePage'));
	...
	public function registerScripts() {
		...
		/* Custom Post */
		wp_enqueue_script( 'rascals-cpt', esc_url( RASCALS_TOOLKIT_URL ) . '/core/cpt/assets/js/admin-custom-post.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable', 'jquery-ui-droppable', 'jquery-ui-draggable'), false, true );
		...
		/* Touch punch */
		wp_enqueue_script( 'jquery-ui-touch-punch', esc_url( RASCALS_TOOLKIT_URL ) . '/core/cpt/assets/vendors/jquery.ui.touch-punch.min.js', false, false, true );
	...
	public function registerStyles() {
		...
		wp_enqueue_style( 'sortable_custom_post_css', esc_url( RASCALS_TOOLKIT_URL ) . '/core/cpt/assets/css/admin-custom-post.css' );
	}
	...
	public function displaySortablePage() {
		...
		echo '<h3>' . rascals_esc_( 'Sort Items' ) . ' <img src="' . esc_url( site_url() ) . '/wp-admin/images/loading.gif" id="loading-animation" alt="' . esc_attr( $this->args['post_name'] ) . '"/></h3>';
		...
		while ( $sort_query->have_posts() ){ 
			...
			echo '<a href="' . esc_url( home_url() ) . '/wp-admin/post.php?action=edit&post=' . esc_attr( get_the_ID() ) . '" class="edit-item" title="' . rascals_esc_( 'Edit Post' ) . '"></a>';
