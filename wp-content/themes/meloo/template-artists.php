...
// Get layout 
$artist_layout = get_post_meta( $wp_query->post->ID, '_artist_layout', true );
$artist_layout = ( $artist_layout !== '' ? $artist_layout : 'wide' );
...
$block = ( $block !== '' ? $block : 'artists-block1' );
$block_option = '';
switch ($block) {
	case 'artists-block1': // grid
		$block = 'artists-block1';
		$block_option = '4';
		break;
	case 'artists-block2': // grid
		$block = 'artists-block1';
		$block_option = '3';
		break;
	case 'artists-block3': // grid
		$block = 'artists-block1';
		$block_option = '2';
		break;
}
...
// Filter 
$filter_args = array(
	'filter_atts'       => get_post_meta( $wp_query->post->ID, '_filter_atts_artists', false ), 
...
if ( $artist_layout === 'narrow' ) {
	array_push( $content_classes, 'page-layout-' . $artist_layout );
	$container_classes[] = 'small';
} else if ( $artist_layout === 'wide' ) {
	array_push( $content_classes, 'page-layout-' . $artist_layout );
	$container_classes[] = 'wide';
} else if ( $artist_layout === 'left_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $artist_layout, 'layout-style-1', 'sidebar-on-left' );
} else if ( $artist_layout === 'right_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $artist_layout, 'layout-style-1', 'sidebar-on-right' );
}
