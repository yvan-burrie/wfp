...
/* ==================================================
  Releated Posts 
================================================== */
if ( ! function_exists( 'meloo_block_rp' ) ) :

function meloo_block_rp( $params = array() ) {
    ...
    if ( $terms ) {
        ...
        if ( $related_posts_q->have_posts() ) {
            ...
            $html .= '<h4 class="block-rp-title h-style-2">' . esc_html__( 'More Posts', 'meloo' ) . '</h4>';

            $html .= '<div class="ajax-posts-slider anim anim-slide-from-right" data-obj=\'{"action": "meloo_posts_slider_ajax", "cats": "", "category__in": "' . esc_attr( $ajax_category_in ) . '", "tag__in": "' . esc_attr( $ajax_tag_in ) . '", "post__not_in": "' . esc_attr( $post_id ) . '", "ignore_sticky_posts": "1", "cpt": "post", "tax": "category", "limit": "' . esc_attr( $limit ) . '", "excerpt": "' . esc_attr( $excerpt ) . '", "thumb_size": "' . esc_attr( $thumb_size ) . '", "module" : "' . esc_attr( $module ) . '" }\' data-pagenum="1">';
