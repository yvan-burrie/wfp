...
// Videos - Click Action 
$ca = get_post_meta( $wp_query->post->ID, '_click_action', true );

// Get layout 
$videos_layout = get_post_meta( $wp_query->post->ID, '_videos_layout', true );
$videos_layout = ( $videos_layout !== '' ? $videos_layout : 'wide' );
...
$block = ( $block !== '' ? $block : 'videos-block1' );
$block_option = '';
switch ($block) {
	case 'videos-block1':
		$block = 'videos-block1';
...
// Filter 
$filter_args = array(
	'filter_atts'       => get_post_meta( $wp_query->post->ID, '_filter_atts_videos', false ), 
...
// Queries  
$filters_query = meloo_prepare_videos_filter( $filter_args );
...
if ( $videos_layout === 'narrow' ) {
	array_push( $content_classes, 'page-layout-' . $videos_layout );
	$container_classes[] = 'small';
} else if ( $videos_layout === 'wide' ) {
	array_push( $content_classes, 'page-layout-' . $videos_layout );
	$container_classes[] = 'wide';
} else if ( $videos_layout === 'left_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $videos_layout, 'layout-style-1', 'sidebar-on-left' );
} else if ( $videos_layout === 'right_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $videos_layout, 'layout-style-1', 'sidebar-on-right' );
}
