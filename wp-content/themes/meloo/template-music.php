...
$releases_layout = ( $releases_layout !== '' ? $releases_layout : 'wide' );
...
$block = ( $block !== '' ? $block : 'music-block1' );
$block_option = '';
switch ($block) {
	case 'music-block1':
		$block = 'music-block1';
...
// Filter 
$filter_args = array(
	'filter_atts'       => get_post_meta( $wp_query->post->ID, '_filter_atts_music', false ), 
...
if ( $releases_layout === 'narrow' ) {
	array_push( $content_classes, 'page-layout-' . $releases_layout );
	$container_classes[] = 'small';
} else if ( $releases_layout === 'wide' ) {
	array_push( $content_classes, 'page-layout-' . $releases_layout );
	$container_classes[] = 'wide';
} else if ( $releases_layout === 'left_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $releases_layout, 'layout-style-1', 'sidebar-on-left' );
} else if ( $releases_layout === 'right_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $releases_layout, 'layout-style-1', 'sidebar-on-right' );
}
