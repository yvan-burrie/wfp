...
$events_layout = ( $events_layout !== '' ? $events_layout : 'wide' );
...
// Queries  
$filters_query = meloo_prepare_events_filter( $filter_args );
...
if ( $events_layout === 'narrow' ) {
	array_push( $content_classes, 'page-layout-' . $events_layout );
	$container_classes[] = 'small';
} else if ( $events_layout === 'wide' ) {
	array_push( $content_classes, 'page-layout-' . $events_layout );
	$container_classes[] = 'wide';
} else if ( $events_layout === 'left_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $events_layout, 'layout-style-1', 'sidebar-on-left' );
} else if ( $events_layout === 'right_sidebar' ) {
	$sidebar = true;
	array_push( $content_classes, 'page-layout-' . $events_layout, 'layout-style-1', 'sidebar-on-right' );
}
